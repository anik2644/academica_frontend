        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <button
            *ngIf="addLabel"
            type="button"
            (click)="add.emit()"
            class="inline-flex shrink-0 items-center gap-2 rounded-2xl bg-white/15 px-4 py-2.5 text-sm font-semibold text-white backdrop-blur-sm transition hover:bg-white/25"
          >
              ...
              <div *ngIf="filteredRows.length; else emptyBlock" class="mt-5">
                  ...
                  <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                      <tr>
                        ...
                        <th *ngIf="showActions" class="px-5 py-4 text-right text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                          Actions
                        </th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 bg-white">
                      <tr *ngFor="let row of filteredRows" class="align-top transition hover:bg-slate-50/80">
                        ...
                        <td *ngIf="showActions" class="px-5 py-4 text-right">
                          <div class="flex items-center justify-end gap-1">
                            <button
                              *ngIf="canView"
                              type="button"
                              (click)="view.emit(row)"
                              class="rounded-xl p-2 text-slate-400 transition hover:bg-blue-50 hover:text-blue-600"
                              title="View Details"
                            >
                              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <button
                              *ngIf="canEdit"
                              type="button"
                              (click)="edit.emit(row)"
                              class="rounded-xl p-2 text-slate-400 transition hover:bg-primary-50 hover:text-primary-600"
                              title="Edit"
                            >
                              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="grid gap-3 lg:hidden">
                  ...
                  >
                    ...
                    <div *ngIf="showActions" class="mt-3 flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                      <button *ngIf="canView" type="button" (click)="view.emit(row)" class="rounded-xl bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 transition hover:bg-blue-100">View</button>
                      <button *ngIf="canEdit" type="button" (click)="edit.emit(row)" class="rounded-xl bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary-700 transition hover:bg-primary-100">Edit</button>
                      <button *ngIf="canDelete" type="button" (click)="delete.emit(row)" class="rounded-xl bg-rose-50 px-3 py-1.5 text-xs font-medium text-rose-700 transition hover:bg-rose-100">Delete</button>
                    </div>
                  </article>
                </div>
              </div>
